...
function App(){
 ...
 <header className="hc-toolbar"><div><span className="hc-eyebrow">INCURISE / SPATIAL STUDIES</span><h1>左右まで広がる、3つの空間。</h1></div><div className="hc-tabs" role="tablist" aria-label="背景を比較">{options.map((o,i)=><button key={o.id} role="tab" id={`space-tab-${i}`} aria-selected={selected===i} aria-controls="space-stage" tabIndex={selected===i?0:-1} onClick={()=>select(i)} onKeyDown={e=>{let n=i;if(e.key==="ArrowRight")n=(i+1)%4;else if(e.key==="ArrowLeft")n=(i+3)%4;else if(e.key==="Home")n=0;else if(e.key==="End")n=3;else return;e.preventDefault();select(n);document.getElementById(`space-tab-${n}`)?.focus()}}><b>{o.id}</b><span>{o.name}</span></button>)}</div></header>
 ...
 <section className="lc-hero" aria-labelledby="space-title" key={`${selected}-${replay}`}><div className="lc-particles"><GeometricHero centerShift={-.24} space={active.space}/></div><div className="lc-shade" aria-hidden="true"/><div className="lc-copy"><p className="lc-label">INCURISE CONSULTING / RECRUIT 2026</p><h2 id="space-title" aria-label="0から1の挑戦を、1から100の成長へ。"><span className="lc-title-line">0<img className="lc-arrow" src={arrow} alt=""/><strong>1</strong>の挑戦を、</span><span className="lc-title-line">1<img className="lc-arrow" src={arrow} alt=""/><strong>100</strong>の成長へ。</span></h2><div className="lc-bottom"><p className="lc-lead">未経験から技術を仕事にする人も、経験を次の事業へつなぐ人も。<br/>一人ひとりの現在地から、成長の続きをつくる。</p><a href="https://incurise.co.jp/about/">私たちを知る<img src={linkArrow} alt=""/></a></div></div><div className="mc-hint">INCUBATE / RISE</div></section></main>
 <footer className="hc-footer"><span>A〜Cが新案、Dが元の背景です。右下で一時停止できます。</span><a href="../motion-comparison/?motion=A">モーション比較へ戻る</a></footer></div>
}
